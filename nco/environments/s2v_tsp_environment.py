...
class S2VTSPNetwork(Network):
    def embed_state(self, vertex_states, ub_vertices):
        with tf.variable_scope("graph_embedding", reuse=True):
            theta_1 = tf.get_variable(name="theta_1")

        # [ES, 1] x [1, NC] = [ES, NC]
        state_embedding = tf.reshape(vertex_states, [1, -1], name="state_reshape")
        state_embedding = tf.matmul(theta_1, state_embedding, name="state_embedding")
    ...
    def pool_neighbourhood(self, embedding, neighbourhoods, theta):
        ...
        pooled_neighbourhood = tf.reshape(pooled_neighbourhood, [EMBEDDING_SIZE, -1],
    ...
    def embed_distances(self, distances, batch_size, ub_vertices):
        ...
        relu_pooled_distances = tf.reshape(relu_pooled_distances, [EMBEDDING_SIZE, batch_size * ub_vertices, ub_vertices],
    ...
    def graph_embedding_layer(self, embedding, vertex_states, neighbourhoods, distances, batch_size, ub_vertices):
        ...
        return tf.reshape(new_embedding, [batch_size, EMBEDDING_SIZE, ub_vertices], name="new_embedding_reshape")
    ...
    def embedding2Qfunction(self, embedding, neighbourhoods):
        ...
        # [ES, ES] x [ES, NC] = [ES, NC]
        embedding = tf.reshape(embedding, [EMBEDDING_SIZE, -1], name="embedding_reshape")
    ...
    def tf_apply(self, x, internals=(), update=False, return_internals=False):
        ...
        neighbourhoods = x["neighbourhoods"]

        batch_size = tf.shape(var_states)[0]

        embedding = tf.zeros((batch_size, EMBEDDING_SIZE, OUTPUT_SIZE), dtype=tf.float32)
        ...
        for it in range(EMBEDDING_ITERATIONS):
            embedding = self.graph_embedding_layer(embedding, var_states, neighbourhoods, distances, batch_size, OUTPUT_SIZE)
            embedding = tf.nn.relu(embedding, name="relu_embedding_" + str(it))

        q_hat = self.embedding2Qfunction(embedding, neighbourhoods)
        q_hat = tf.reshape(q_hat, [batch_size, OUTPUT_SIZE], name="q_hat_reshaped")
...
class S2VTSPEnvironment():
    def __init__(self, instance_type="uniform"):
        self.network = S2VTSPNetwork
        ...
        self.states = dict(vertex_states=dict(type="float", shape=(OUTPUT_SIZE)),
                           distances=dict(type="float", shape=(OUTPUT_SIZE, OUTPUT_SIZE)),
                           neighbourhoods=dict(type="float", shape=(OUTPUT_SIZE, OUTPUT_SIZE)))
