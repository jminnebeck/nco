...
class S2CONVTSPNetwork(Network):
    def tf_apply(self, x, internals=(), update=False, return_internals=False):
        ...
        batch_size = tf.shape(vertex_states)[0]

        embedding = tf.zeros((batch_size, EMBEDDING_SIZE, EMBEDDING_SIZE, 2 + OUTPUT_SIZE), dtype=tf.float32)
...
class S2CONVTSPEnvironment():
    def __init__(self, instance_type="uniform"):
        self.network = S2CONVTSPNetwork
        ...
        self.states = dict(vertex_states=dict(type="float", shape=(OUTPUT_SIZE)),
                           coordinates=dict(type="float", shape=(OUTPUT_SIZE, 2)),
    ...
    def get_state(self):
        return dict(vertex_states=self.vertex_states,
                    coordinates=np.asarray(self.vertices),
    ...
    def draw_random_instance(self, compare_solvers=False):
        ...
        self.coordinates = np.asarray(self.vertices)
        self.coordinates = ((self.coordinates - np.min(self.coordinates)) * EMBEDDING_SIZE) \
                           // ((np.max(self.coordinates) - np.min(self.coordinates)) * EMBEDDING_SIZE)
